...
def calculate_area(pred, label, num_classes, ignore_index=255):
    ...
    label = label * mask.astype(label.dtype)
...
def get_f1(intersect_area, pred_area, label_area):
    intersect_area = intersect_area.numpy()
    pred_area = pred_area.numpy()
    label_area = label_area.numpy()
    precision = intersect_area/pred_area
    recall = intersect_area/label_area
    f1 = 2*(precision*recall)/(precision+recall)
    # f1 = paddle.nan_to_num(f1)

    # f1[paddle.isnan(f1)] = 0
    f1 = np.nan_to_num(f1)
    f1_mean = np.mean(f1)
    return f1, float(f1_mean)
    # class_f1_sco = []
    ...
    # return np.array(class_f1_sco), float(f1[1].numpy())
...
def fw_iou(intersect_area, pred_area, label_area):
    ...
    # print(fw)
    FWiou = np.sum(fw* iou)
    # FWiou = (fw[fw > 0] * iou[fw > 0]).sum()
    return np.array(class_iou), miou, FWiou
