...
class OCRNet_M(nn.Layer):
    ...
    def __init__(self,
        ...
        self.backbone = backbone
        self.backbone_indices = backbone_indices
        in_channels = [self.backbone.feat_channels[i]*3 for i in backbone_indices]
    ...
    def forward(self, x1, x2):
        ...
        # feats1 = [feats1[i] for i in self.backbone_indices]
        # feats2 = [feats2[i] for i in self.backbone_indices]
        # featsbin = [paddle.abs(feats1[i]-feats2[i]) for i in self.backbone_indices]
        featsbin = [paddle.concat([paddle.abs(feats1[i]-feats2[i]), feats1[i], feats2[i]], axis=1) for i in self.backbone_indices]
...
class OCRNet_P(nn.Layer):
    ...
    def __init__(self,
        ...
        self.backbone_indices = backbone_indices
        in_channels = [self.backbone.feat_channels[i] for i in backbone_indices]
        in_channelsaug = [self.backbone.feat_channels[i]*2 for i in backbone_indices]
        ...
        self.head = OCRHead(
            ...
            ocr_mid_channels=ocr_mid_channels,
            ocr_key_channels=ocr_key_channels)
        self.headaug = OCRHead(
            num_classes=num_classes,
            in_channels=in_channelsaug,
            ocr_mid_channels=ocr_mid_channels,
            ocr_key_channels=ocr_key_channels)
    ...
    def forward(self, x1, x2):
        ...
        # feats2 = [feats2[i] for i in self.backbone_indices]
        featsbin = [paddle.abs(feats1[i]-feats2[i]) for i in self.backbone_indices]
        featsaug = [paddle.concat([feats1[i], feats2[i]], axis=1) for i in self.backbone_indices]
        logit_list = self.head(featsbin)
        logit_listaug = self.headaug(featsaug)
        logit_list.extend(logit_listaug)
        # if not self.training:
        #     logit_list = [logit_list[0]]
