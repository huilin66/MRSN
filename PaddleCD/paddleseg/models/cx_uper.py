...
@manager.MODELS.add_component
class CX_Uper_4B_PMRG_V2(CX_Uper_4B):
    def __init__(self, **kwargs):
...
class PixelModalityReliabilityGateV2(nn.Layer):
    """Baseline-preserving pixel reliability gate.

    Uniform gates reproduce direct concat. Learned gate deviations then amplify
    reliable modalities and suppress unreliable ones at each pixel.
    """
    ...
    def forward(self, feats):
        ...
        for i, feat in enumerate(feats):
            gate = gates[:, i:i + 1, :, :]
            centered_gate = self.num_modalities * gate - 1.0
            scale = self.scale[i].reshape([1, 1, 1, 1])
            alpha = self.alpha[i].reshape([1, 1, 1, 1])
            outs.append(feat * (1.0 + scale * centered_gate) + alpha * gate * self.enhance[i](feat))
