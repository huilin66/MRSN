...
def read_image_miou_sheet(summary_path: Path, sheet_name: str, rank_column: str, topk: int):
    try:
...
def load_model_assets(ana_dir: Path) -> list[ModelAssets]:
    assets = []
...
def save_gt_color(label_path: str, output_path: Path):
    ...
    label_img.putpalette(get_color_map_list(256))
...
def collect_sample(sample: SelectedSample, ana_dir: Path, output_dir: Path, model_assets: list[ModelAssets], args):
    ...
    try:
        save_rgb_image(sample.image1_path, rgb_path, args.rgb_bands)
        manifest_rows.append({"type": "rgb", "path": str(rgb_path), "source": sample.image1_path})
    except Exception as exc:
        manifest_rows.append({"type": "rgb", "path": "", "source": sample.image1_path, "error": str(exc)})

    try:
        save_gt_color(sample.label_path, gt_color_path)
        manifest_rows.append({"type": "gt_color", "path": str(gt_color_path), "source": sample.label_path})
    except Exception as exc:
...
def main() -> int:
    ...
    samples = read_image_miou_sheet(summary_path, args.sheet, args.rank_column, args.topk)
    if not samples:
        print("ERROR: no valid samples found for rank column {}".format(args.rank_column), file=sys.stderr)
        return 4

    model_assets = load_model_assets(ana_dir)
    summary_rows = []
